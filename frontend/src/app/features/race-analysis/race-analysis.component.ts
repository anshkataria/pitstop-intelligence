...
@Component({
  ...
  template: `<app-intelligence-shell><section class="screen">
    <header class="screen-head"><div><p class="eyebrow">ANALYSIS</p><h1>Race Review</h1><p>Grid and classification, compared from the official historical record.</p></div>
      ...
      <select [(ngModel)]="round" (ngModelChange)="loadResults()">@for(race of races();track race.id){<option [ngValue]="race.round" [disabled]="!isCompleted(race.round)">R{{race.round}} · {{race.name}}{{isCompleted(race.round)?'':' · Upcoming'}}</option>}</select></div>
    </header>
